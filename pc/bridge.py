...
Anti-eco half-duplex (--gate, attivo di default insieme a --return):
...
Uso:  python bridge.py [--host ronaldo.local] [--return "Cuffie (Oculus"] [--gate mute|duck|off]
...
def main():
    ...
    ap.add_argument("--gate", choices=["mute", "duck", "off"], default="mute",
                    help="anti-eco: mentre il ritorno trasmette, il mic di Emiglio viene azzerato (mute), "
                         "attenuato (duck) o lasciato stare (off). Default mute; ignorato senza --return")
    ...
    if args.ret:
        ...
        print(f"[gate] modalita' {args.gate}" + (f", hold {args.gate_hold}s" if args.gate != "off" else "")
